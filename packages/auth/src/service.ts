...
export class AuthService {
  ...
  async login(input: { email: string; password: string; ipAddress?: string; userAgent?: string }): Promise<AuthSession> {
    ...
    const { data, error } = await this.database.client.auth.signInWithPassword({
  ...
  async resolve(accessToken: string, organizationId?: string) {
    ...
    const { data, error } = await this.database.client.auth.getUser(accessToken);
  ...
  async refresh(refreshToken: string): Promise<AuthSession> {
    ...
    const { data, error } = await this.database.client.auth.refreshSession({ refresh_token: refreshToken });
    if (error || !data.session || !data.user) throw new AuthenticationError();
    const expiresAt = data.session.expires_at
      ? new Date(data.session.expires_at * 1000)
      : new Date(Date.now() + 60 * 60 * 1000);
  ...
  async logout(accessToken: string): Promise<void> {
    ...
    const userResult = await this.database.client.auth.getUser(accessToken);
    if (userResult.error || !userResult.data.user) return;
    const { error } = await this.database.client.auth.admin.signOut(userResult.data.user.id, "global");
